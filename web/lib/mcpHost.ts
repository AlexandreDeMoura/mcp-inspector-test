...
import type { Server, MCPTool, MCPToolWithServer } from './types';
import {
  MCP_SERVERS,
...
// Global singleton pattern to survive hot-reloads
const globalStore = globalThis as unknown as {
  _mcp_connectedServers: Map<string, ConnectedServer>;
  _mcp_healthCheckInterval: NodeJS.Timeout | null;
};

if (!globalStore._mcp_connectedServers) {
  globalStore._mcp_connectedServers = new Map<string, ConnectedServer>();
}

// We don't initialize healthCheckInterval here because we want to start it explicitly

const connectedServers = globalStore._mcp_connectedServers;
...
export async function connectServer(definition: ServerDefinition): Promise<ConnectedServer> {
  ...
  // Define createServerFromDefinition locally if not exported properly or just use inline logic
  // The imported one should work if I copied mcp-servers.ts correctly.
  // Wait, I missed copying createServerFromDefinition in the previous step? 
  // Let me double check mcp-servers.ts content I wrote. 
  // Ah, I see I missed `createServerFromDefinition` in my write to mcp-servers.ts (I only see the arrays and getters).
  // I will add a local helper here to be safe or fix mcp-servers.ts later. 
  // Actually, let's fix mcp-servers.ts after this. For now I'll implement a helper here.
...
export function startHealthChecks(intervalMs: number = 30000): void {
  if (globalStore._mcp_healthCheckInterval) {
    clearInterval(globalStore._mcp_healthCheckInterval);
  }

  globalStore._mcp_healthCheckInterval = setInterval(() => {
...
export function stopHealthChecks(): void {
  if (globalStore._mcp_healthCheckInterval) {
    clearInterval(globalStore._mcp_healthCheckInterval);
    globalStore._mcp_healthCheckInterval = null;
